...
<Steps>
  <Step title="Get an API Key">
    ...
    <Info>
      The bootstrap endpoint is the **only** endpoint that uses `Authorization: Bearer`.
      All other PolySimulator API endpoints authenticate with the `X-API-Key` header.
    </Info>
    ...
    **Response (201 Created):**
    ...
    <Warning>
      **Save `raw_key` immediately** — it is shown only once and cannot be
      retrieved again. Only the SHA-256 hash is stored server-side.
    </Warning>
