...
<Steps>
  ...
  <Step title="Place Your First Trade">
    ...
    with PolySimClient() as client:                 # reads POLYSIM_API_KEY from env
        market = client.list_markets(limit=1, hot_only=True)[0]   # an actively-traded market
    ...
    <CodeGroup>
      ...
      MARKET_ID=$(curl -s -H "X-API-Key: $POLYSIM_API_KEY" \
        "$POLYSIM_BASE_URL/v1/markets?hot_only=true&limit=1" | jq -r '.[0].condition_id')
      ...
      market_id = requests.get(f"{base}/v1/markets?hot_only=true&limit=1", headers=h).json()[0]["condition_id"]
      ...
      const markets = await (await fetch(`${base}/v1/markets?hot_only=true&limit=1`, { headers: h })).json();
