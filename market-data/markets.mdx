...
| `volume_24h` | `string \| null` | Rolling 24-hour volume in USD (stringified for precision). **`null` means "no data yet"** (this market has not been observed yet); `"0"` means real zero. |
...
<Note>
  **Data freshness**: `volume_24h`, `last_price`, and `category` are
  cached display values refreshed periodically. Newly-listed markets
  surface immediately with these fields as `null` and gain real values
  once data becomes available. Treat `null` as "data pending" rather
  than "missing market".
</Note>
