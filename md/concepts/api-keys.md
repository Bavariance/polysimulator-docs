...
| `403` | `POST /v1/keys`, `POST /v1/keys/bootstrap` | `TIER_REQUIRES_UPGRADE` if a free key requested `trade`. Residual `CLOSED_BETA` / `API_PRO_COMING_SOON` only on an emergency close. Branch on `X-Polysim-Code`. |
...
if resp.status_code == 201:
    ...
    # (the body's `error` is the human message).
