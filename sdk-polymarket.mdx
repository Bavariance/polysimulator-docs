---
title: "Polymarket py-sdk Mirror"
sidebarTitle: "py-sdk Mirror"
description: "polysim_polymarket — a paper-mode mirror of Polymarket's unified py-sdk (polymarket-client). Sync + async PublicClient/SecureClient: swap the import prefix, host, and auth and your py-sdk bot runs on paper."
---
...
<CardGroup cols={2}>
  ...
  <Card title="py-sdk compatibility" icon="arrows-left-right" href="/concepts/py-sdk-compatibility">
...
<Note>
  The install name is **`polysimulator`**. It ships three import surfaces in one
  package — `polysim_sdk` (the native client), `polysim_clob_client` (the
  `py-clob-client` v1 drop-in), and `polysim_polymarket` (this py-sdk mirror).
  See the [Python SDK overview](/sdk) for how the three relate.
</Note>
...
client = PublicClient(host="https://api.polysimulator.com", api_key="ps_live_…")
```
...
<CardGroup cols={2}>
  <Card title="Seam contract & compat matrix" icon="arrows-left-right" href="/concepts/py-sdk-compatibility">
    What paper mode does and doesn't do, per surface area — read before you rely on anything.
  </Card>
  <Card title="Native polysim_sdk" icon="python" href="/sdk">
