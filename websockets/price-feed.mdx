...
Connect to `/v1/ws/prices?token=<jwt>` with a short-lived token minted from [`POST /v1/keys/ws-token`](/authentication). Upon connection, the server immediately sends an initial greeting frame:
