...
<Info>
  **Primary auth**: The API primarily uses `X-API-Key` for authentication. The one-time
  [bootstrap endpoint](/quickstart) (`POST /v1/keys/bootstrap`) instead accepts a Supabase
  JWT via `Authorization: Bearer` to create your first key. Key management endpoints
  also accept Bearer JWT where explicitly noted.
</Info>
