---
title: "Polymarket Raw-HTTP Endpoints"
sidebarTitle: "Polymarket Raw HTTP"
description: "Drop-in compatibility with Polymarket's CLOB HTTP API — change only the host."
---
...
<Warning>
  **Status enum split between write and read endpoints.** The
  ...
  returns PM's `OpenOrder.status` enum — PM's **exact** members since
  the 2026-06-11 PM-compat batch:
  ...
  **Migration (2026-06-11):** before this date the read side emitted
  invented members (`ORDER_STATUS_PENDING` / `ORDER_STATUS_FILLED` /
  double-L `ORDER_STATUS_CANCELLED` / `ORDER_STATUS_REJECTED`) that
  real PM never sends. Update any string matches; the `?status=`
...
Up to **100** ids per request (PM's documented cap is 3,000; we cap
lower for cohort stability — raise post-launch when load patterns
stabilise). Invalid ids appear in `not_canceled` with an explanation.
...
client = ClobClient(
    ...
    key="<your-private-key>",                # leave as-is (ignored)
)
